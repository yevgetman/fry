...
func PlanningExecutiveFromUserPromptPrompt(userPrompt, mediaManifest, assetsSection string) string {
...
- If supplementary asset documents are provided below, incorporate their context into your executive summary — they contain reference material that informs the initiative's scope, constraints, and goals.
...
func PlanningStep0Prompt(executiveContent, mediaManifest, assetsSection string) string {
...
- If supplementary asset documents are provided below, use them as reference material for planning methodology and analytical framework decisions. Incorporate relevant specifications, requirements, and constraints from those documents into the plan.
...
func PlanningStep2Prompt(planContent, agentsContent, epicExamplePath, userPrompt string, effort epic.EffortLevel, enableReview bool, mediaManifest, assetsSection string) string {
	userPromptLine := ""
	if userPrompt != "" {
...
func effortSizingGuidancePlanning(effort epic.EffortLevel) string {
	...
	case epic.EffortFast:
		return `
EFFORT LEVEL: FAST
The user has indicated this is a fast planning task. You MUST:
...
The user has indicated this is a standard planning task. You MUST:
...
- If the plan describes a simple, well-bounded task (1-2 documents): use FAST effort (1-2 sprints, @effort fast)
- If the plan describes a moderate analysis (3-5 documents): use STANDARD effort (2-4 sprints, @effort standard)
...
func PlanningOverviewPrompt(planContent, executiveContent, userPrompt string, effort epic.EffortLevel, mediaManifest, assetsSection string) string {
	return buildOverviewPrompt("senior strategic planner", planContent, executiveContent, userPrompt, effort, mediaManifest, assetsSection)
}
...
func PlanningStep3Prompt(planContent, epicContent, verificationExamplePath, userPrompt, mediaManifest string) string {
	...
	return fmt.Sprintf(`You are generating a verification.md (sanity checks) file for an autonomous AI planning system.

This is a PLANNING project — the AI agent produces structured documents, NOT code. Sanity checks must validate that document deliverables exist and contain the required content.

Read these files carefully:
1. %s — The FORMAT REFERENCE showing exact syntax and check primitives. IGNORE the software-specific examples — adapt the primitives for document sanity checks.

...
func TestAuditMetricsRecordAndSummary(t *testing.T) {
	...
	metrics.Record(CallMetric{SessionType: engine.SessionAuditFix, PromptBytes: 120, DurationMs: 20, WasNoOp: true})
	...
	assert.Equal(t, int64(60), metrics.TotalDurationMs())
	assert.InDelta(t, 1.0, metrics.NoOpRate(), 0.001)
	assert.InDelta(t, 2.0, metrics.VerifyYield(), 0.001)
	assert.Equal(t, 100, metrics.AvgPromptBytes())
}

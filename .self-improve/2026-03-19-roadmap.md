...
- **Description:** All build output is currently unstructured terminal text. A `--json-report` flag would emit a `build-report.json` containing sprint results, sanity check outcomes, alignment attempt counts, audit findings, and timing data. The data already exists scattered across `.fry/` files.
- **Implementation:**
...
- **Implementation:**
  ...
  6. Update `docs/sanity-checks.md` with `@check_test` documentation
...
- **Effort:** Medium (~2 hours)
- **Description:** There's currently no visibility into how many tokens each sprint, alignment attempt, or audit cycle consumes. Adding token tracking would help users optimize effort levels and model selection.
- **Implementation:**
...
### D3. Targeted alignment (fix specific check categories)

- **Impact:** Medium — reduces token waste in alignment loops
- **Effort:** Medium (~2 hours)
- **Description:** Currently the alignment loop sends ALL failed checks to the agent every iteration. When a sprint has 10 failed checks of different types, the agent tries to fix everything at once and often fails. Targeted alignment would prioritize by severity and fix one category per iteration.
- **Implementation:**

...
run_build_phase() {
    ...
    if [ "$build_success" = true ]; then
        log "Running post-build verification (make test && make build && make install)..."
        local test_output
        test_output="$(cd "$WORKTREE_DIR" && make test 2>&1 && make build 2>&1 && make install 2>&1)" || true

        if (cd "$WORKTREE_DIR" && make test && make build && make install) >/dev/null 2>&1; then
            ...
            while [ "$heal_attempt" -lt "$MAX_POST_BUILD_HEALS" ]; do
                ...
                failure_output="$(cd "$WORKTREE_DIR" && make test 2>&1; make build 2>&1; make install 2>&1)" || true
...
4. Run 'make test && make build && make install' to verify your fix before finishing.
...
HEALPROMPT
                ...
                # Re-verify
                if (cd "$WORKTREE_DIR" && make test && make build && make install) 2>&1 | tee -a "$LOG_FILE"; then
...
- [ ] \`make install\` installs the binary system-wide

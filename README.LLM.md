...
For each sprint (startSprint → endSprint):
 ...
 10. Sprint audit (if enabled & effort != fast) — two-level loop:
     ...
     │  ├─ Fix loop validates each pass against a diff contract (issue IDs, target files, expected evidence), rejects empty/comment-only/out-of-scope diffs, routes already-fixed no-op claims to verify, and excludes blocker-category findings from normal code-fix iterations

...
- **Remediation clustering:** Before each fix pass, Fry groups related findings into coherent remediation clusters using shared file families, subsystem paths, and requirement-theme overlap. The fix prompt targets those clusters first while verify still checks issue IDs one-by-one.

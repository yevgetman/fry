...
#   REFERENCES    → "Read .fry/AGENTS.md, then [plan sections]."
...
# @effort <fast|standard|high|max>  Effort level — controls sprint count and density
...
# @review_model <model>           Model for reviewer session (default: same as @model)
# @max_deviation_scope <N>        Max sprints a single deviation can touch (default: 3)
...
Read .fry/AGENTS.md first, then plans/plan.md [relevant sections].

[Project context: naming conventions, module paths, key decisions.]

IMPORTANT: .fry/AGENTS.md already exists with critical project rules. Do NOT create, overwrite, or modify .fry/AGENTS.md. It is read-only for all sprints.
...
===PROMISE: SPRINT1_DONE=== when [specific exit criteria].
...
Read .fry/AGENTS.md [relevant rules], then plans/plan.md [relevant sections].
...
===PROMISE: SPRINT2_DONE=== when [exit criteria].
...
Read .fry/AGENTS.md, then plans/plan.md [relevant sections].
...
===PROMISE: SPRINT3_DONE=== when [exit criteria].

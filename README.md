...
- **Self-healing** -- automatic re-runs with targeted fix prompts on verification failure; `--retry` picks up where a failed build left off with boosted heal attempts; `--continue` uses an LLM agent to analyze build state and auto-resume

...
After a sprint finishes, fry runs **verification checks** (`@check_file`, `@check_file_contains`, `@check_cmd`, `@check_cmd_output`). If any fail, `run_heal_loop()` (line 611) kicks in.

## Step 1: Collect failures (`collect_failed_checks`, line 535)
...
## Step 2: Build the heal prompt (lines 632–659)

The prompt is written to `prompt.md` (overwriting the sprint prompt) and has this structure:
...
## Step 3: Run the agent (lines 664–676)
...
## Step 4: Re-verify and loop (lines 684–698)
...
- The failure report is **appended to `progress.txt`** so the next heal attempt has context about what was already tried
...
3. **Accumulating context** — Each failed heal attempt appends its failure report to `progress.txt`, so subsequent attempts know what was already tried and what's still broken
4. **Same agent, fresh context** — Each heal attempt is a fresh agent invocation reading the same `prompt.md` file, but with richer `progress.txt` context from prior attempts

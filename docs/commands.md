...
| `--heuristic` | Used with `--continue`. Skip the LLM analysis and use a fast heuristic instead: scan sprint completion markers in the build state, return the first sprint without a marker as the resume point (default: `false`). No LLM call is made. Useful in CI/CD or when LLM analysis is unavailable. |

...
| `fry_build_restart` | Restart a failed/stopped build with `--continue`, `--resume`, or `--simple-continue`. |
...
The `internal/agent/` package is designed as the foundation for a native `fry agent` command that does not require OpenClaw. The path:

1. **Done (Layer 0)**: Domain types, state readers, event streaming, artifact schema, prompt generation
2. **Future**: LLM client (`internal/agent/llm/`), channel adapters (`internal/agent/channels/`), `fry agent` command

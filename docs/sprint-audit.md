...
1. **Per-cluster unified fix dispatch** -- Fry clusters findings by shared file family, subsystem, and requirement theme. Each fix iteration targets **one cluster at a time** with a unified prompt that carries the full audit context (codebase description, git diff, sprint progress, resolved themes) alongside the per-cluster fix instructions and inline target file content. This gives the fix agent the same understanding the audit agent had when it discovered the issues, eliminating context transfer loss while keeping fixes scoped to one cluster. If a cluster's fix is rejected (no-op, comment-only, or out-of-scope), all its findings are **fast-fail skipped** for remaining iterations in that cycle.

...
- Sprint audits use progress-based iteration: continue as long as the fix agent resolves issues or uncovers new ones, up to a safety cap of 10. Stops early if 2 consecutive passes show no progress.
...
- Sprint audits use progress-based iteration (same as `high`): continue while the fix agent is making progress, up to a safety cap of 10
...
| Sprint audit | Skipped | 3 iterations (bounded) | Progress-based (cap 10) | Progress-based (cap 10) |
